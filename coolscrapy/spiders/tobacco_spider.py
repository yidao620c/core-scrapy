...
CREATE TABLE `t_tobacco` (
  ...
  `product_type`              VARCHAR(16)  COMMENT '产品类型',
  `package_spec`              VARCHAR(32)  COMMENT '包装规格',
...
class TobaccoSpider(Spider):
    ...
    def parse(self, response):
        ...
        for item in self.parse_page(response):
            yield item
        # 找下一页链接递归爬
        next_url = response.xpath('//a[text()="【下一页】"]/@href').extract()[0]
    ...
    def parse_page(self, response):
        ...
        for ind, each_row in enumerate(response.xpath('//div[@id="mainlist"]/table/tbody/tr')):
            ...
            item['product_type'] = each_row.xpath('td[3]/text()').extract()[0]
            item['package_spec'] = each_row.xpath('td[4]/text()').extract()[0]
            item['reference_price'] = each_row.xpath('td[5]/span/text()').extract()[0]
            item['manufacturer'] = each_row.xpath('td[6]/text()').extract()[0]
